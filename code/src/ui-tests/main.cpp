...
static Gdiplus::Color rgb(Byte r, Byte g, Byte b)
{
...
public:
	WidgetContext()
	{
		S32 const render_box_count = 4096;
		render_items.items = reinterpret_cast<RenderItem*>(malloc(sizeof(RenderItem) * render_box_count));
		render_items.count = render_box_count;

		S32 const cursor_interact_box_count = 512;
		cursor_interact_boxes.items = reinterpret_cast<CursorInteractBox*>(malloc(sizeof(CursorInteractBox) * cursor_interact_box_count));
		cursor_interact_boxes.count = cursor_interact_box_count;
	}
	...
	void frame_reset(Float2 root_clip_size)
	{
		...
		render_items[0] = RenderItem{
			.type = RenderItem::Type::ClipRect,
			.clip_rect = {
				.position = Float2{},
				.size = root_clip_size,
			},
		};
		current_clip_rect = &render_items[0].clip_rect;
		render_item_index = 1;
	...
	{
		PAW_ASSERT(current_clip_rect != nullptr, "current_clip_rect should always have a value. By default it should be the size of the client");
		RenderClipRect const* const previous_clip_rect = current_clip_rect;

		RenderItem& item = render_items[render_item_index++];
		item.type = RenderItem::Type::ClipRect;
		item.clip_rect = clip_rect;
...
private:
	...
	RenderClipRect const* current_clip_rect = nullptr;
};
...
{
	char const* name;
	Gdiplus::Color color = rgb(255, 255, 255);
...
{
	Gdiplus::Color const color = state.color % 2 == 0 ? rgb(255, 0, 0) : rgb(0, 255, 0);
	state.color++;
...
static void gui(RECT const& client_rect)
{
	// fprintf(stdout, "==================== Frame Start =================\n");
	Float2 const ui_size = {
		.x = static_cast<F32>(client_rect.right - client_rect.left),
		.y = static_cast<F32>(client_rect.bottom - client_rect.top),
	};
	...
	Gdiplus::Color const background_base = Gdiplus::Color(255, 230, 230, 230);
	Gdiplus::Color const background_layer1(255, 248, 248, 248);
	Gdiplus::Color const background_layer2(255, 255, 255, 255);
			...
			})
			{
				...
				PADDING({10.0f, 10.0f, 10.0f, 10.0f})
				{
					BOX({
						...
						.height = Size::fill(),
						.color = Gdiplus::Color(255, 120, 187, 250),
					});
				}
				...
				FLEX_ITEM({})
				{
					...
					})
					{
						FLEX_ITEM({0.75f})
						{
							...
							{
								Gdiplus::Color const button_color(255, 213, 213, 213);
								for (int i = 0; i < 7; i++)
								{
									Gdiplus::Color menu_item_color = button_color;
						...
						FLEX_ITEM({1.25f})
						{
							...
							PADDING({0.0f, 0.0f, 10.0f, 0.0f})
							{
								...
								Slice<TabState* const> tabs = ConstSliceFromInitList({&first_tab, &second_tab, &third_tab, &fourth_tab});
					...
					})
					{
						...
						FLEX_ITEM({})
						{
							BOX({
								.width = Size::fill(),
								.height = Size::fill(),
								.color = Gdiplus::Color(255, 184, 199, 214),
							})
							{
								PADDING({10.0f, 10.0f, 10.0f, 10.0f})
								{
...
static HDC g_backbuffer_hdc = NULL;
...
static LRESULT CALLBACK window_proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	switch (msg)
	{
		case WM_PAINT:
		{
			...
			LONG const client_width = client_rect.right - client_rect.left;
			LONG const client_height = client_rect.bottom - client_rect.top;

			Gdiplus::Graphics graphics(g_backbuffer_hdc);
			// Gdiplus::SolidBrush brush(Gdiplus::Color(255, 0, 0, 255));
			// graphics.FillRectangle(&brush, 100, 100, 300, 400);
			g_graphics = &graphics;
			gui(client_rect);

			g_background_widget_context.render(graphics);
			g_foreground_widget_context.render(graphics);

			PAINTSTRUCT ps{};
			HDC hdc = BeginPaint(hwnd, &ps);
			BitBlt(hdc, 0, 0, client_width, client_height, g_backbuffer_hdc, 0, 0, SRCCOPY);
			EndPaint(hwnd, &ps);
		}
		break;
		...
		case WM_SIZE:
		{
			...
			g_backbuffer_hdc = CreateCompatibleDC(NULL);
			g_backbuffer_bitmap = CreateCompatibleBitmap(GetDC(hwnd), client_width, client_height);
			SelectObject(g_backbuffer_hdc, g_backbuffer_bitmap);
		}
		break;

		case WM_MOUSEMOVE:
		{
			// RedrawWindow(hwnd, nullptr, 0, RDW_NOERASE | RDW_INTERNALPAINT | RDW_INVALIDATE);
			POINT mouse_pos_point{};
			GetCursorPos(&mouse_pos_point);
			ScreenToClient(hwnd, &mouse_pos_point);
			Float2 const mouse_pos{static_cast<F32>(mouse_pos_point.x), static_cast<F32>(mouse_pos_point.y)};
			g_foreground_widget_context.reset_mouse_move();
			g_background_widget_context.reset_mouse_move();
			g_element_tree->process_mouse_move(Float2{}, mouse_pos);
		...
		case WM_LBUTTONDOWN:
		{
			...
			g_element_tree->process_mouse_bkutton_down(Float2{}, MouseButton::Left, mouse_pos);
			RedrawWindow(hwnd, NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW);
		}
		break;
		...
		case WM_LBUTTONUP:
		{
			g_switched = !g_switched;
			g_element_tree->process_mouse_button_up(MouseButton::Left);
			g_active_element_owner = nullptr;
			RedrawWindow(hwnd, NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW);
		}
		break;
		...
		case WM_MOUSEWHEEL:
		{
			F32 const wheel_delta = (F32)GET_WHEEL_DELTA_WPARAM(wparam) / (F32)WHEEL_DELTA;
			g_element_tree->process_mouse_wheel(Float2{0.0f, -wheel_delta * 5.0f});
			RedrawWindow(hwnd, NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW);
			// printf("Wheel delta: %g\n", wheel_delta);
		}
		break;
		...
		case WM_NCACTIVATE:
		{
			...
			if (!(composition_enabled && result == S_OK))
			{
				return 1;
			}
		}
...
{
	Slice<int const> ints = ConstSliceFromInitList({10, 3, 4});
	(void)ints;
	test_main(arg_count, args);
	ArenaAllocator widget_allocator{};
	g_widget_allocator = &widget_allocator;

	ElementTree element_tree{};
	g_element_tree = &element_tree;

	ULONG_PTR gdi_token;
	Gdiplus::GdiplusStartupInput gdi_startup_input;
	Gdiplus::GdiplusStartup(&gdi_token, &gdi_startup_input, nullptr);
	...
	WNDCLASSEX const wc{
		...
		.lpszMenuName = "",
		.lpszClassName = "UI-Test",
		.hIconSm = 0,
	};

	RegisterClassEx(&wc);
	// HWND first_handle = NULL;
	{

		DWORD const window_style = WS_POPUP | WS_THICKFRAME | WS_CAPTION | WS_SYSMENU | WS_MAXIMIZEBOX | WS_MINIMIZEBOX;
		// DWORD const window_style = WS_OVERLAPPEDWINDOW;
		HWND window_handle = CreateWindowEx(0, wc.lpszClassName, "UI-Test", window_style, CW_USEDEFAULT, CW_USEDEFAULT, 1600, 900, NULL, NULL, wc.hInstance, NULL);
		SetWindowLongPtr(window_handle, GWL_STYLE, static_cast<LONG>(window_style));
		// first_handle = window_handle;
		ShowWindow(window_handle, SW_SHOW);
		UpdateWindow(window_handle);

		BOOL composition_enabled = FALSE;
		HRESULT result = DwmIsCompositionEnabled(&composition_enabled);
		PAW_ASSERT(composition_enabled == TRUE && result == S_OK, "Win32 Composition is not enabled");
		PAW_ASSERT_UNUSED(composition_enabled);

		static constexpr MARGINS shadow_state{1, 1, 1, 1};
		result = DwmExtendFrameIntoClientArea(window_handle, &shadow_state);
		PAW_ASSERT(result == S_OK, "DwmExtendFrameIntoClientArea failed");

		// Trigger NCCALCSIZE
		SetWindowPos(window_handle, 0, 0, 0, 0, 0, SWP_FRAMECHANGED | SWP_NOMOVE | SWP_NOSIZE);
	}
	...
	//	DWORD const window_style = WS_POPUP | WS_THICKFRAME | WS_CAPTION | WS_SYSMENU | WS_MAXIMIZEBOX | WS_MINIMIZEBOX;
	//	// DWORD const window_style = WS_OVERLAPPEDWINDOW;
	//	HWND window_handle = CreateWindowEx(0, wc.lpszClassName, "UI-Test", window_style, CW_USEDEFAULT, CW_USEDEFAULT, 800, 450, NULL, NULL, wc.hInstance, NULL);
	//	SetWindowLongPtr(window_handle, GWL_STYLE, static_cast<LONG>(window_style));
	...
	// SetWindowPos(first_handle, 0, 0, 0, 0, 0, SWP_FRAMECHANGED | SWP_NOMOVE | SWP_NOSIZE);
